...
functions {
   matrix gp_matern_12_cov(vector a, vector b, real alpha, real rho) {
      int M = num_elements(a);
      int N = num_elements(b);
  
      // Replicate a and b into matrices for pairwise operations
      matrix[M, N] diff_matrix = rep_matrix(a, N)' - rep_matrix(b, M);

      // Take the element-wise absolute value
      return alpha^2 * exp(-abs(diff_matrix)/ rho);
   }
}
...
model {
  ...
  matrix[N_obs, N_obs] cov =  gp_matern_12_cov(xs, xs, alpha, rho)
...
generated quantities {
  ...
  {
    matrix[N_obs, N_obs] K_x1 = gp_matern_12_cov(xs, xs, alpha, rho);
