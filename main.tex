...
First, it is noteworthy that visually all considered methods were able to
mean predict the simulated processes satisfyingly.
Figure~\ref{fig:smooth} shows how an example from each process was inferred by
all of the considered methods. The predicted mean produced by all methods
followed the simulated processes accurately, although the LPR appears to
underfit for some data sets. In addition to that, towards the ends of the
simualted time-series the GP regression sometimes tends towards zero (which
is a property of the squared exponential kernel used) and the polynomial
regressions appears to overfit with excessive uncertainty.
